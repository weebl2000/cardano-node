...
getBlockTxs (ShelleyBlock shelleyEra Consensus.ShelleyBlock{Consensus.shelleyBlockRaw})
  ...
  where
    ...
    go = case shelleyBlockRaw of
           Shelley.Block _header (Shelley.TxSeq txs) -> [ShelleyTx shelleyEra (error "TODO") | _tx <- toList txs]
           -- TODO: Does Consensus need to switch to Core.Tx in Shelley.Block?
